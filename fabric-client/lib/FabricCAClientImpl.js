				...
				response.on('end', function () {
					...
					try {
						...
						if (enrollResponse.success) {
                            ...
                            //return resolve(new Buffer.from((enrollResponse.result).toString(), 'base64'));
                            //return resolve(new Buffer.from((enrollResponse.result).toString(), 'base64').toString());
                            //return resolve(enrollResponse.result);
                            return resolve(new Buffer.from((enrollResponse.result.Cert), 'base64').toString());
						} else {
							return reject(new Error(
